...
void Player::do_vibrato(int p_track,bool fine) {
	...
	if ((control.ticks_counter==0) && control.channel[p_track].row_has_note) control.channel[p_track].vibrato_position=0;
        ...
        switch (control.channel[p_track].vibrato_type) {
                ...
                case 2: /* ramp down */
                        ...
                        break;
                case 3: /* random */
                        temp=random_generate(&control.random_seed) %256;//getrandom(256);
                        break;
        }
...
void Player::do_pitch_slide_down(int p_track,Uint8 inf) {
	...
	Uint8 hi,lo;
        ...
        if (hi==0xf) {
                ...
                if (!control.ticks_counter) control.channel[p_track].aux_period+=(Uint16)lo<<2;
        } else if (hi==0xe) {

                if (!control.ticks_counter) control.channel[p_track].aux_period+=lo;
        } else {

                if (control.ticks_counter) control.channel[p_track].aux_period+=(Uint16)inf<<2;
        }
}

void Player::do_pitch_slide_up(int p_track,Uint8 inf) {
...
void Player::do_pitch_slide_to_note(int p_track) {
        ...
        if (control.ticks_counter) {
            ...
            /* if they are equal or if portamentospeed is too big... */
                if ((!dist)||((control.channel[p_track].portamento_speed<<2)>intabs(dist))) {
...
void Player::do_tremor(int p_track) {
...
void Player::do_pan_slide(int p_track) {
...
void Player::do_volume_slide(int p_track,int inf) {
...
void Player::do_channel_volume_slide(int p_track) {
...
void Player::do_tremolo(int p_track) {
...
void Player::do_arpegio(int p_track) {
	...
	if (dat) {
		...
		if (song->has_linear_slides()) {
			...
			control.channel[p_track].period=control.channel[p_track].aux_period-intabs(get_period((Uint16)46,0)-get_period((Uint16)44,0))*note;			
		} else if (control.channel[p_track].sample_ptr) {
			
			control.channel[p_track].period=get_period( (((Uint16)control.channel[p_track].note)+note)<<1,SampleManager::get_singleton()->get_c5_freq( (control.channel[p_track].sample_ptr->get_sample_data())));			
                }
...
void Player::do_retrig(int p_track) {
...
void Player::do_global_volume_slide(int p_track) {
...
void Player::do_panbrello(int p_track) {
...
void Player::do_effect_S(int p_track) {
		...
		//Also not supported!	
			...
			switch(inf) {
				...
				case 0x4:  {
					...
					control.channel[p_track].NNA_type=Instrument::NNA_NOTE_CONTINUE;
				} break;
				case 0x5:  {
					
					control.channel[p_track].NNA_type=Instrument::NNA_NOTE_OFF;
				} break;
				case 0x6:  {
					
					control.channel[p_track].NNA_type=Instrument::NNA_NOTE_FADE;
				} break;
				case 0x7:  {
...
void Player::run_volume_column_effects(int p_track) {
...
void Player::run_effects(int p_track) {
...
void Player::pre_process_effects() {
