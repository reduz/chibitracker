...
void Reverb::process(float *p_src,float *p_dst,int p_frames) {
	...
	int predelay_frames=(int)((params.predelay/1000.0)*params.mix_rate);
	...
	if (params.hpf>0) {
		float hpaux=expf(-2.0*PI_VALUE*params.hpf*6000/params.mix_rate);
	...
	for (int i=0;i<MAX_COMBS;i++) {
		...
		int size_limit=c.size-(int)((float)c.extra_spread_frames*(1.0-params.extra_spread));
	...
	for (int i=0;i<MAX_ALLPASS;i++) {
		...
		AllPass &a=allpass[i];
		ap_size_limit[i]=a.size-(int)((float)a.extra_spread_frames*(1.0-params.extra_spread));
	}
...
#define PROCESS_ALLPASS(m_ap) 	\
	...
	*/
	...
	for (int i=0;i<MAX_ALLPASS;i++) {
		...
		AllPass &a=allpass[i];
		int size_limit=a.size-(int)((float)a.extra_spread_frames*(1.0-params.extra_spread));
...
void Reverb::configure_buffers() {
	...
	for (int i=0;i<MAX_COMBS;i++) {
		...
		c.extra_spread_frames=(int)(params.extra_spread_base*params.mix_rate);
		
		int len=(int)(comb_tunings[i]*params.mix_rate)+c.extra_spread_frames;
		if (len<5)
	...
	for (int i=0;i<MAX_ALLPASS;i++) {
		...
		a.extra_spread_frames=(int)(params.extra_spread_base*params.mix_rate);

		int len=(int)(allpass_tunings[i]*params.mix_rate)+a.extra_spread_frames;
		if (len<5)
			len=5; //may this happen?
...
void Reverb::update_parameters() {
	...
	for (int i=0;i<MAX_COMBS;i++) {
		...
		c.damp=expf(-2.0*PI_VALUE*auxdmp*10000/params.mix_rate); // 0 .. 10khz
	}
