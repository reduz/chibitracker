...
void Interface::set_default_keybinds() {
	...
	KeyBind::set(KB_PATTERN_CURSOR_INSERT_SCRIPT,KEY_MASK_SHIFT|KEY_1);
...
void Interface::interface_update_timer_callback() {
...
void Interface::song_menu_selected(int p_option) {
...
void Interface::config_menu_selected(int p_option) {
...
void Interface::help_menu_selected(int p_option) {
...
void Interface::new_song_accept() {
...
void Interface::song_file_callback(String p_fname) {
...
void Interface::quit_accept() {
	
	
	quit=true;
}

bool Interface::must_quit() {
	
	return quit;
}
void Interface::quit_request() {
	
	confirm_quit_window->show();	
}


void Interface::pre_screen_changed_from(int p_which) {
...
void Interface::screen_changed_to(int p_which) {
...
void Interface::main_key(unsigned long p_unicode, unsigned long p_scan_code,bool p_press,bool p_repeat,int p_modifier_mask) {
...
void Interface::check_instrument_visibility() {
	
	tabs->set_tab_visible( 2, tracker.song->has_instruments() );
	instrument_screen->update_components();
}
Tracker * Interface::get_tracker() {
	
	return &tracker;
}

void Interface::update_components() {
...
void Interface::button_rw_pressed() {
...
void Interface::button_stop_pressed(){
...
void Interface::button_fw_pressed(){
	
	tracker.player->goto_next_order();
}
void Interface::button_play_pattern_pressed(){
...
void Interface::button_play_order_pressed() {
	
	tracker.player->play_start_song_from_order( tracker.editor->orderlist_get_cursor_y() );	
}

void Interface::button_play_cursor_pressed() {
...
void Interface::default_octave_changed(double p_to) {
	
	tracker.editor->set_default_octave( (int) p_to );
}
void Interface::instrument_mask_changed(int p_to) {
...
void Interface::ui_repaint_interval_changed(int p_to) {
...
void Interface::show_splash() {
	
	MessageBox *initial=new MessageBox(window,Window::SIZE_TOPLEVEL_CENTER) ;
	initial->get_custom_vb()->add(new TextView("This is the FIRST ChibiTracker public release. As such, it may still cotnain some bugs.. If you happen to find one (ore more), please send a bug report to reduzio@gmail.com.\n Help make ChibiTracker stable!"),1);
	initial->show("");	
}

Interface::Interface(Window *p_window,Mixer *p_mixer,ConfigApi *p_config) : tracker(p_mixer) {
	...
	window->key_unhandled_signal.connect( this, &Interface::main_key );
	...
	file->add_separator();
	file->add_item("Quit");
	file->item_selected_signal.connect(this, &Interface::song_menu_selected );
	file->set_focus_mode( FOCUS_NONE );
	top_bar->add( new VSeparator,0);
	
	
	top_bar->add( new PlaybackButton( PlaybackButton::TYPE_REWIND ), 0 )->pressed_signal.connect( this, &Interface::button_rw_pressed);
	top_bar->add( new PlaybackButton( PlaybackButton::TYPE_PLAY ), 0 )->pressed_signal.connect( this, &Interface::button_play_pressed);
	top_bar->add( new PlaybackButton( PlaybackButton::TYPE_STOP ), 0 )->pressed_signal.connect( this, &Interface::button_stop_pressed);
	top_bar->add( new PlaybackButton( PlaybackButton::TYPE_FORWARD ), 0 )->pressed_signal.connect( this, &Interface::button_fw_pressed);
	top_bar->add( new VSeparator,0);
	top_bar->add( new PlaybackButton( PlaybackButton::TYPE_PLAY_PATTERN ), 0 )->pressed_signal.connect( this, &Interface::button_play_pattern_pressed);
	top_bar->add( new PlaybackButton( PlaybackButton::TYPE_PLAY_CURSOR ), 0 )->pressed_signal.connect( this, &Interface::button_play_cursor_pressed);
	
	top_bar->add( new VSeparator,0);
	
	
	top_bar->add( new Label("Octave"),0 );
	default_octave=top_bar->add( new SpinBox,0);
	default_octave->set_minimum_size( Size( 50, 0 ) );
	default_octave->get_range()->set_max(8);
	default_octave->get_line_edit()->set_focus_mode( FOCUS_NONE );
	default_octave->get_range()->value_changed_signal.connect(this,&Interface::default_octave_changed);
	top_bar->add( new VSeparator,0);
	
	vu=top_bar->add(new MainVU,1);
	
//	top_bar->add( new Widget,1);
	
	top_bar->add( new VSeparator,0);
	settings=top_bar->add(new MenuBox("Settings"),0);
	settings->add_item("Audio Driver");
	settings->add_item("Software Mixer");
	settings->add_item("Keyboard");
	settings->add_item("Interface");
	settings->add_item("Default Paths");
	settings->item_selected_signal.connect( this, &Interface::config_menu_selected );
	...
	help->item_selected_signal.connect( this, &Interface::help_menu_selected );
	...
	tabs=vbc->add( new TabBar, 0 );
	tabs->add_tab("Patterns");
	tabs->add_tab("Samples");
	tabs->add_tab("Instruments");
	tabs->add_tab("Variables");
	...
	tabs->tab_changed_signal.connect( this, &Interface::screen_changed_to );
	tabs->pre_tab_changed_signal.connect( this, &Interface::pre_screen_changed_from );
	
	pattern_screen=main_stack->add( new PatternScreen(&tracker) );
	
	sample_screen=main_stack->add( new SampleScreen(&tracker) );
	
	instrument_screen = main_stack->add(new InstrumentScreen(&tracker));
	instrument_screen->sample_list_changed_signal.connect( sample_screen, &SampleScreen::update_components );
	
	
	variables_screen = main_stack->add(new VariablesScreen(&tracker));
	variables_screen->song_name_changed_signal.connect(this, &Interface::update_title_bar);
	
	variables_screen->instrument_mode_changed.connect( this, &Interface::check_instrument_visibility );
	...
	new_song_vb->add( new CenterContainer, 0 )->set( new Button("Proceed"))->pressed_signal.connect(this,&Interface::new_song_accept);
	...
	cleanup_vb->add( new CenterContainer, 0 )->set( new Button("Remove"))->pressed_signal.connect(this,&Interface::cleanup_accept);
	...
	quit_vb->add( new CenterContainer, 0)->set( new Button("Exit") )->pressed_signal.connect( this, &Interface::quit_accept );
	...
	ui_dialog->repaint_interval_changed_signal.connect(this, &Interface::ui_repaint_interval_changed );
	...
	window->skin_changed_signal.connect(this,&Interface::skin_changed);
	update_title_bar();
	setup_logos();
